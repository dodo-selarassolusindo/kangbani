...
class Akun extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->matauang_id->Lookup = new Lookup($this->matauang_id, 'matauang', false, 'id', ["kode","nama","",""], '', '', [], [], [], [], [], [], false, '', '', "CONCAT(COALESCE(`kode`, ''),'" . ValueSeparator(1, $this->matauang_id) . "',COALESCE(`nama`,''))");
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "akunview" => $Language->phrase("View"),
            "akunedit" => $Language->phrase("Edit"),
            "akunadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "AkunView",
            Config("API_ADD_ACTION") => "AkunAdd",
            Config("API_EDIT_ACTION") => "AkunEdit",
            Config("API_DELETE_ACTION") => "AkunDelete",
            Config("API_LIST_ACTION") => "AkunList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("akunview", $parm);
        } else {
            $url = $this->keyUrl("akunview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("akunlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("akunlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("akundelete", $parm);
        }
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'akun');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'akun', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'akun', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'akun', $fldname, $key, $oldvalue);
            }
        }
    }
