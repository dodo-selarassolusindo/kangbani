 ...
 * Page class
 */
class KonversiAdd extends Konversi
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "KonversiAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "konversiadd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("konversilist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "konversilist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "konversiview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "konversilist") {
                            ...
                            $returnUrl = "konversilist"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'operasi' first before field var 'x_operasi'
        $val = $CurrentForm->hasValue("operasi") ? $CurrentForm->getValue("operasi") : $CurrentForm->getValue("x_operasi");
        if (!$this->operasi->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->operasi->Visible = false; // Disable update for API request
            } else {
                $this->operasi->setFormValue($val, true, $validate);
            }
        }
    ...
    protected function getAddRow()
    {
        ...
        // operasi
        $this->operasi->setDbValueDef($rsnew, $this->operasi->CurrentValue, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("konversilist"), "", $this->TableVar, true);
