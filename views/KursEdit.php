...
<span id="el_kurs_matauang_id">
    <select
        ...
        data-select2-id="fkursedit_x_matauang_id"
        <?php } ?>
        data-table="kurs"
...
<?php if (!$Page->matauang_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fkursedit", function() {
    var options = { name: "x_matauang_id", selectId: "fkursedit_x_matauang_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fkursedit.lists.matauang_id?.lookupOptions.length) {
        options.data = { id: "x_matauang_id", form: "fkursedit" };
    } else {
        options.ajax = { id: "x_matauang_id", form: "fkursedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
