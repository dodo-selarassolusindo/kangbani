...
class Periode extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // isaktif
        $this->isaktif = new DbField(
            $this, // Table
            'x_isaktif', // Variable name
            'isaktif', // Name
            '`isaktif`', // Expression
            '`isaktif`', // Basic search expression
            200, // Type
            1, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`isaktif`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->isaktif->setDataType(DataType::BIT);
        $this->isaktif->Lookup = new Lookup($this->isaktif, 'periode', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->isaktif->OptionCount = 2;
        $this->isaktif->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['isaktif'] = &$this->isaktif;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "periode";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->isaktif->DbValue = $row['isaktif'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("periodelist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "periodeview" => $Language->phrase("View"),
            "periodeedit" => $Language->phrase("Edit"),
            "periodeadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PeriodeView",
            Config("API_ADD_ACTION") => "PeriodeAdd",
            Config("API_EDIT_ACTION") => "PeriodeEdit",
            Config("API_DELETE_ACTION") => "PeriodeDelete",
            Config("API_LIST_ACTION") => "PeriodeList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("periodeview", $parm);
        } else {
            $url = $this->keyUrl("periodeview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("periodeedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("periodelist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("periodeadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("periodelist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("periodedelete", $parm);
        }
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'periode');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'periode', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'periode', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'periode', $fldname, $key, $oldvalue);
            }
        }
    }
