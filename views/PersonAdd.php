...
$Page->showMessage();
?>
<form name="fpersonadd" id="fpersonadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_person_type_id">
    <select
        ...
        data-select2-id="fpersonadd_x_type_id"
        <?php } ?>
        data-table="person"
...
<?php if (!$Page->type_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fpersonadd", function() {
    var options = { name: "x_type_id", selectId: "fpersonadd_x_type_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpersonadd.lists.type_id?.lookupOptions.length) {
        options.data = { id: "x_type_id", form: "fpersonadd" };
    } else {
        options.ajax = { id: "x_type_id", form: "fpersonadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_person_klasifikasi_id">
    <select
        ...
        data-select2-id="fpersonadd_x_klasifikasi_id"
        <?php } ?>
        data-table="person"
...
<script>
loadjs.ready("fpersonadd", function() {
    var options = { name: "x_klasifikasi_id", selectId: "fpersonadd_x_klasifikasi_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpersonadd.lists.klasifikasi_id?.lookupOptions.length) {
        options.data = { id: "x_klasifikasi_id", form: "fpersonadd" };
    } else {
        options.ajax = { id: "x_klasifikasi_id", form: "fpersonadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpersonadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpersonadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
