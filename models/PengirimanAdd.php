 ...
 * Page class
 */
class PengirimanAdd extends Pengiriman
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PengirimanAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "pengirimanadd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("pengirimanlist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "pengirimanlist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "pengirimanview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "pengirimanlist") {
                            ...
                            $returnUrl = "pengirimanlist"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'akunbeli' first before field var 'x_akunbeli'
        $val = $CurrentForm->hasValue("akunbeli") ? $CurrentForm->getValue("akunbeli") : $CurrentForm->getValue("x_akunbeli");
        if (!$this->akunbeli->IsDetailKey) {
            if (IsApi() && $val === null) {
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if (!CheckInteger($this->tipe->FormValue)) {
                $this->tipe->addErrorMessage($this->tipe->getErrorMessage(false));
            }
    ...
    protected function getAddRow()
    {
        ...
        // akunbeli
        $this->akunbeli->setDbValueDef($rsnew, $this->akunbeli->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['akunbeli'])) { // akunbeli
            $this->akunbeli->setFormValue($row['akunbeli']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("pengirimanlist"), "", $this->TableVar, true);
